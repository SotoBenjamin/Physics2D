...
sf::Vector2f toSfmlVec(const Vec2 &v) { return {v.x, v.y}; }
Vec2 fromSfmlVec(const sf::Vector2f &v) { return {v.x, v.y}; }
...
void resetScene() {
  ...
  physicsEngine.addHalfSpaceStaticObject(
      {WINDOW_WIDTH / 2.0f, (float)WINDOW_HEIGHT - 20.f}, {0.0f, -1.0f});
  physicsEngine.addHalfSpaceStaticObject(
      {(float)WINDOW_WIDTH - 20.f, WINDOW_HEIGHT / 2.0f}, {-1.0f, 0.0f});

  auto boxId1 =
      physicsEngine.addBody({900.f, (float)WINDOW_HEIGHT - 20.f - 25.0f / 2.0f},
                            createBoxCollider({150.f, 25.f}));
  auto boxId2 = physicsEngine.addBody(
      {1050.f, physicsEngine.bodies[boxId1].motionState.pos.y},
      createBoxCollider({150.f, 25.f}));
  auto boxId3 = physicsEngine.addBody(
      {850.f, physicsEngine.bodies[boxId2].motionState.pos.y - 25.0f / 2.0f -
                  50.0f / 2.0f},
      createBoxCollider({25.f, 50.f}));
  auto boxId4 = physicsEngine.addBody(
      {1100.f, physicsEngine.bodies[boxId3].motionState.pos.y},
      createBoxCollider({25.f, 50.f}));
  auto boxId5 = physicsEngine.addBody(
      {900.f, physicsEngine.bodies[boxId4].motionState.pos.y - 50.0f / 2.0f -
                  25.0f / 2.0f - 2},
      createBoxCollider({150.f, 25.f}));
  auto boxId6 = physicsEngine.addBody(
      {1050.f, physicsEngine.bodies[boxId5].motionState.pos.y},
      createBoxCollider({150.f, 25.f}));
  auto boxId7 = physicsEngine.addBody(
      {975.f, physicsEngine.bodies[boxId5].motionState.pos.y - 25.0f / 2.0f -
                  50.0f / 2.0f},
      createBoxCollider({25.f, 50.f}));

  structureBodyIds = {boxId1, boxId2, boxId3, boxId4, boxId5, boxId6, boxId7};
  ...
  physicsEngine.bodies[birdId].motionState.mass =
      50.f; // Hacemos al pájaro más pesado
  physicsEngine.bodies[birdId].flags.setKinematic(
      true); // Lo hacemos cinemático hasta que se lance
}

int main() {
  sf::RenderWindow window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT),
                          "Simulacion Estilo Angry Birds");
  window.setFramerateLimit(120);

  physicsEngine.simulationPhysicsSettings.gravity = {0.0f, 400.0f};
  physicsEngine.collisionChecksCount =
      24; // Más iteraciones para pilas estables
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      ...
      if (event.type == sf::Event::MouseButtonReleased &&
        ...
        if (isDragging) {
          ...
          physicsEngine.bodies[birdId].motionState.velocity =
              fromSfmlVec(dragVector) * 2.f;
          for (auto &body : physicsEngine.bodies) {
            physicsEngine.bodies[body.first].flags.setKinematic(false);
          }
        }
      }
    }
    ...
    if (isDragging) {
      sf::Vector2f birdPos =
          toSfmlVec(physicsEngine.bodies[birdId].motionState.pos);
